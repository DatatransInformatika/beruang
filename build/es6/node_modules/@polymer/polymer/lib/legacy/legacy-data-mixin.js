...
this.constructor=UndefinedArgumentError;this.__proto__=UndefinedArgumentError.prototype}};/**
 ...
 */export const LegacyDataMixin=dedupingMixin(superClass=>{/**
   ...
   */class LegacyDataMixin extends superClass{/**
     ...
     * @private
     */_marshalArgs(args,path,props){const vals=super._marshalArgs(args,path,props);// Per legacy data rules, single-property observers (whether in `properties`
...
if(this._legacyUndefinedCheck&&1<vals.length){for(let i=0;i<vals.length;i++){if(vals[i]===void 0){// Break out of effect's control flow; will be caught in
// wrapped property effect function below
const name=args[i].name;throw new UndefinedArgumentError(`Argument '${name}' is undefined.`,name)}}}return vals}/**
     ...
     * @return {void}
     * @protected
     */_addPropertyEffect(property,type,effect){return super._addPropertyEffect(property,type,wrapEffect(effect,effect&&effect.info&&effect.info.methodName))}/**
     * Overrides `Polyer.PropertyEffects` to wrap effect functions to
     * catch `UndefinedArgumentError`s and warn.
     *
     ...
     * @return {void}
     * @protected
     */static _addTemplatePropertyEffect(templateInfo,prop,effect){return super._addTemplatePropertyEffect(templateInfo,prop,wrapEffect(effect))}}return LegacyDataMixin});// LegacyDataMixin is applied to base class _before_ metaprogramming, to
// ensure override of _addPropertyEffect et.al. are used by metaprogramming
// performed in _finalizeClass
Polymer.Class=(info,mixin)=>Class(info,superClass=>mixin?mixin(LegacyDataMixin(superClass)):LegacyDataMixin(superClass));// Apply LegacyDataMixin to Templatizer instances as well, and defer
 ...
 */const TemplatizeMixin=dedupingMixin(superClass=>{/**
     ...
     * @private
     */class TemplateLegacy extends legacyBase{get _legacyUndefinedCheck(){return this._methodHost&&this._methodHost._legacyUndefinedCheck}}/** @type {!Polymer_PropertyEffects} */TemplateLegacy.prototype._methodHost;return TemplateLegacy});templatize.mixin=TemplatizeMixin;console.info("LegacyDataMixin will be applied to all legacy elements.\n"+"Set `_legacyUndefinedCheck: true` on element class to enable.");