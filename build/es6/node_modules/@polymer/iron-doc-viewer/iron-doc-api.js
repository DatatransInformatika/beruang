...
*/import"../polymer/polymer-legacy.js";import"./iron-doc-function.js";import"./iron-doc-hide-bar.js";import"./iron-doc-property.js";import"./iron-doc-viewer-styles.js";import{Polymer}from"../polymer/lib/legacy/polymer-fn.js";import{html}from"../polymer/lib/utils/html-tag.js";import{IronDocViewerBehavior}from"./iron-doc-viewer-behavior.js";Polymer({_template:html`
...
`,is:"iron-doc-api",behaviors:[IronDocViewerBehavior],properties:{descriptor:Object,_showProtectedProperties:Boolean,_showProtectedMethods:Boolean,_showProtectedEvents:Boolean,_staticMethods:{computed:"_computeStaticMethods(descriptor)"},_hasMethods:{computed:"_computeHasMethods(descriptor)"}},_anyVisible:function(items){for(var i=0,item;i<(items||[]).length;i++){item=items[i];if(!item.privacy||"public"===item.privacy||"protected"===item.privacy){return!0}};return!1},_public:function(items){for(var filtered=[],i=0,item;i<(items||[]).length;i++){item=items[i];if(!item.privacy||"public"===item.privacy){filtered.push(item)}}return filtered},_protected:function(items){for(var filtered=[],i=0,item;i<(items||[]).length;i++){item=items[i];if("protected"===item.privacy){filtered.push(item)}}return filtered},_protectedCount:function()/** arguments */{for(var sum=0,i=0;i<arguments.length;i++){sum+=this._protected(arguments[i]).length}return sum},_computeStaticMethods:function(descriptor){return descriptor.staticMethods||[]},_computeHasMethods:function(descriptor){return descriptor.methods&&0<descriptor.methods.length||descriptor.staticMethods&&0<descriptor.staticMethods.length}});