...
for(i=0;i<info.removedNodes.length;i++){let el=info.removedNodes[i],primaryValue=el.obj[this.key],index=this._recordIndex(primaryValue);if(0<=index){this.splice("recordset",index,1)}}};this._observer=new FlattenedNodesObserver(slot,fMonitorCallback)}addTuple(index,tuple){let primaryValueArr=this._primaryValueArray(),recordLen=this.recordset.length;this._addTuple(index,tuple,primaryValueArr,recordLen)}addTuples(index,tuples){let primaryValueArr=this._primaryValueArray(),recordLen=this.recordset.length;for(let i=0,primaryValue;i<tuples.length;i++){primaryValue=this._addTuple(index++,tuples[i],primaryValueArr,recordLen++);primaryValueArr.push(primaryValue)}}removeTuple(id){let index=this._recordIndex(id);if(0<=index){this.splice("recordset",index,1)}}removeTuples(ids){for(let i=0;i<ids.length;i++){this.removeTuple(ids[i])}}sortTuples(fSort){//this.recordset.sort(fSort);
let len=this.recordset.length;for(let i=0;i<len-1;i++){let championIndex=i,championRecord=this.recordset[i],championTuple=this._mergeTuple(championRecord),challengerIndex=i+1,challengerRecord=this.recordset[i+1],challengerTuple=this._mergeTuple(challengerRecord);for(let j=i+2;j<len;j++){let record=this.recordset[j],tuple=this._mergeTuple(record);if(0<fSort(challengerTuple,tuple)){challengerIndex=j;challengerRecord=record;challengerTuple=this._mergeTuple(record)}}if(0<fSort(championTuple,challengerTuple)){this.recordset[championIndex]=challengerRecord;this.recordset[challengerIndex]=championRecord}}this.notifyPath("recordset")}_addTuple(index,tuple,primaryValueArr,recordLen){let primaryValue=this._checkTuple(tuple,primaryValueArr),record=this._createRecord(tuple);if(0>index||index>=recordLen){this.push("recordset",record)}else{this.splice("recordset",index,0,record)}return primaryValue}_createRecord(tuple){return{tuple:tuple,changeset:{}}}_recordIndex(primaryValue){let index=-1;for(let i=0;i<this.recordset.length;i++){let record=this.recordset[i],value=this._recordValue(record,this.key);if(primaryValue==value){index=i;break}}return index}_recordValue(record,key){return record.changeset.hasOwnProperty(key)?record.changeset[key]:record.tuple[key]}_mergeTuple(record){let ks=Object.keys(record.tuple),tuple={};for(let i=0,k;i<ks.length;i++){k=ks[i];tuple[k]=this._recordValue(record,k)}return tuple}_checkTuple(tuple,primaryValueArr){if(!tuple.hasOwnProperty(this.key)){throw new ReferenceError("tuple must have key property ["+this.key+"]")}let primaryValue=tuple[this.key],index=primaryValueArr.indexOf(primaryValue);if(0<=index){throw new ReferenceError("duplicate primary value ["+primaryValue+"]")}return primaryValue}_primaryValueArray(){let primaryValueArr=[];for(let i=0,value;i<this.recordset.length;i++){value=this._recordValue(this.recordset[i],this.key);primaryValueArr.push(value)}return primaryValueArr}}window.customElements.define("black-store",BlackStore);