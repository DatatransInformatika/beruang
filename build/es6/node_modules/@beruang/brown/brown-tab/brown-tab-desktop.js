...
element=el;while(sibling=element.previousElementSibling){if("BROWN-TABITEM-DESKTOP"==sibling.nodeName){if(!callback||callback(sibling)){return sibling}}element=sibling}return null}_getContentByDataTab(tabId){return this._getContent(cnt=>{let data=cnt.getAttribute("data-tab");return data==tabId})}_getContent(callback){let el=this.firstElementChild;while(el){if("BROWN-TABITEM-DESKTOP"!=el.nodeName){if(callback(el)){return el}}el=el.nextElementSibling}return null}}window.customElements.define("brown-tab-desktop",BrownTabDesktop);