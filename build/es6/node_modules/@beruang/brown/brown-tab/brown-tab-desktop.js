import{html,PolymerElement}from"../../../@polymer/polymer/polymer-element.js";import{GestureEventListeners}from"../../../@polymer/polymer/lib/mixins/gesture-event-listeners.js";import{FlattenedNodesObserver}from"../../../@polymer/polymer/lib/utils/flattened-nodes-observer.js";import{afterNextRender}from"../../../@polymer/polymer/lib/utils/render-status.js";import*as Gestures from"../../../@polymer/polymer/lib/utils/gestures.js";import"../../../@polymer/iron-flex-layout/iron-flex-layout.js";import"../../../@webcomponents/shadycss/entrypoints/apply-shim.js";import{BeruangBehavior}from"../../beruang-behavior.js";import{BrownBehavior}from"../brown-behavior.js";import"../brown-style.js";class BrownTabDesktop extends BrownBehavior(BeruangBehavior(GestureEventListeners(PolymerElement))){static get template(){return html`
    ...
    `}static get properties(){return{dynamic:{type:Boolean,value:!1,reflectToAttribute:!0},content:{type:String,value:"static",/*options: static, dynamic*/reflectToAttribute:!0}}}ready(){super.ready();Gestures.addListener(this,"tap",e=>{//v
...
let tabItem=e.target;if("static"==this.content){let content=this._getContentByDataTab(tabItem.id);//tabItem.id "item1", "item2", "item3", etc...
...
let fCheck=el=>{return"content"==el.slot},content=this._getLightDOM(this.firstElementChild,!0,fCheck);if(content){this.removeChild(content)}}_requestDynamicContent(id){//v
...
return tabId==data};return this._getLightDOM(this.firstElementChild,!0,fCheck)}_getLightDOM(startEl,forward,callback){//v
let el=startEl;while(el){if(callback(el)){//x
return el;//x
}el=forward?el.nextElementSibling:el.previousElementSibling}return null}_ensureItemVisible(tabItem){//v
...
return"BROWN-TABITEM-DESKTOP"==el.nodeName});if(tabItem){let left=tabItem.offsetLeft-container.scrollLeft;visible=0>left}}if(visible){scroller.classList.add("scroll")}else{scroller.classList.remove("scroll")}}_preScroll(e){//v
