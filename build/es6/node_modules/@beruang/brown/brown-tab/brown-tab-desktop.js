...
_getContentByDataTab(tabId){return this._getLightDOM(el=>{let data="BROWN-TABITEM-DESKTOP"!=el.nodeName?el.getAttribute("data-tab"):null;return tabId==data})}_getLightDOM(callback){let el=this.firstElementChild;while(el){if(callback(el)){return el}el=el.nextElementSibling}return null}_getLightDOMReverse(callback){let el=this.lastElementChild;while(el){if(callback(el)){return el}el=el.previousElementSibling}return null}_ensureItemVisible(tabItem){let container=this.shadowRoot.querySelector("#header-item"),right=tabItem.offsetLeft+tabItem.offsetWidth-container.scrollLeft,left=tabItem.offsetLeft-container.scrollLeft,scroll=0;if(right>container.offsetWidth){scroll=right-container.offsetWidth}else if(0>left){scroll=left}if(0!=scroll){container.scrollLeft+=scroll;this._evaluateScroll()}}_evaluateScroll(){setTimeout(()=>{this._evaluateScrollDo("#post-scroll");this._evaluateScrollDo("#pre-scroll")},100)}_evaluateScrollDo(scrollerId){let container=this.shadowRoot.querySelector("#header-item"),scroller=this.shadowRoot.querySelector(scrollerId),visible=!1;if("#post-scroll"==scrollerId){let tabItem=this._getLightDOMReverse(el=>{//get last tabItem
return"BROWN-TABITEM-DESKTOP"==el.nodeName});if(tabItem){let right=tabItem.offsetLeft+tabItem.offsetWidth-container.scrollLeft;visible=right>container.offsetWidth}}else if("#pre-scroll"==scrollerId){let tabItem=this._getLightDOM(el=>{//get first tabItem
return"BROWN-TABITEM-DESKTOP"==el.nodeName});if(tabItem){let left=tabItem.offsetLeft-container.scrollLeft;visible=0>left}}if(visible){scroller.classList.add("scroll")}else{scroller.classList.remove("scroll")}}_preScroll(e){let tabItem=this._getLightDOM(el=>{//search first tabItem
return"BROWN-TABITEM-DESKTOP"==el.nodeName});if(tabItem){let container=this.shadowRoot.querySelector("#header-item"),offset=tabItem.offsetLeft-container.scrollLeft;if(0>offset){offset=Math.max(offset,-1*(container.offsetWidth/5));container.scrollLeft+=offset;this._evaluateScroll()}}}_postScroll(e){let tabItem=this._getLightDOMReverse(el=>{//search last tabItem
