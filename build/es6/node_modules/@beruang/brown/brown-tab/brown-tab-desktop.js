...
}el.active=!0}}break}}});this.addEventListener("active-changed",e=>{let content=this._getContentByDataTab(e.target.id);if(!content){return}if(e.detail.active){content.classList.add("content-active")}else{content.classList.remove("content-active")}})}_getSibling(el,callback){let sibling,element=el;//forward
...
element=el;while(sibling=element.previousElementSibling){if("BROWN-TABITEM-DESKTOP"==sibling.nodeName){if(!callback||callback(sibling)){return sibling}}element=sibling}return null}_getContentByDataTab(tabId){return this._getContent(cnt=>{let data=cnt.getAttribute("data-tab");return data==tabId})}_getContent(callback){let el=this.firstElementChild;while(el){if("BROWN-TABITEM-DESKTOP"!=el.nodeName){let ok=callback(el);if(ok){return el}}el=el.nextElementSibling}return null}}window.customElements.define("brown-tab-desktop",BrownTabDesktop);