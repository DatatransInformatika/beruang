...
*/class BrownTabManualDesktop extends BrownTabBehavior(BrownBehavior(BeruangBehavior(GestureEventListeners(PolymerElement)))){static get template(){return html`
    ...
    `}ready(){super.ready();let slot=this.shadowRoot.querySelector("#content"),fMonitorCallback=info=>{for(let i=0;i<info.addedNodes.length;i++){let content=info.addedNodes[i],activeTabId=content.getAttribute("data-active-tab"),nextActiveTab=activeTabId?this._getTabItemByDataTab(activeTabId):null;if(nextActiveTab){let oldActiveTab=this._getActiveTabItemBy();if(nextActiveTab!=oldActiveTab){if(oldActiveTab){oldActiveTab.active=!1}nextActiveTab.active=!0}}}//for(let i=0;i<info.addedNodes.length;i++)
};this._contentObserver=new FlattenedNodesObserver(slot,fMonitorCallback)}_onTabClosing(tabItem){this.dispatchEvent(new CustomEvent("manual-tabitem-closing",{bubbles:!0,composed:!0,detail:{tabItem:tabItem,oldContent:this._getContent(),nextSibling:this._getTabItemNextSibling(tabItem),prevSibling:this._getTabItemPrevSibling(tabItem)}}))}_onTabActiving(tabItem){this.dispatchEvent(new CustomEvent("manual-tabitem-activing",{bubbles:!0,composed:!0,detail:{id:tabItem.id,oldContent:this._getContent()}}))}_onTabActiveChanged(tabItem,active){}_getActiveTabItemBy(){let fCheck=el=>{let isQualified="BROWN-TABITEM-DESKTOP"==el.nodeName&&el.active;return isQualified};return this._getLightDOM(this.firstElementChild,!0,fCheck)}_getTabItemByDataTab(tabId){let fCheck=el=>{let data="BROWN-TABITEM-DESKTOP"==el.nodeName?el.id:null;return tabId==data};return this._getLightDOM(this.firstElementChild,!0,fCheck)}_getTabItemNextSibling(tabItem){let fCheck=el=>{return"BROWN-TABITEM-DESKTOP"==el.nodeName};return this._getLightDOM(tabItem.nextElementSibling,!0,fCheck)}_getTabItemPrevSibling(tabItem){let fCheck=el=>{return"BROWN-TABITEM-DESKTOP"==el.nodeName};return this._getLightDOM(tabItem.previousElementSibling,!1,fCheck)}_getContent(){let fCheck=el=>{return"content"==el.getAttribute("slot")};return this._getLightDOM(this.lastElementChild,!1,fCheck)}}window.customElements.define("brown-tab-manual-desktop",BrownTabManualDesktop);