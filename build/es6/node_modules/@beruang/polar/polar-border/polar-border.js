...
*/class PolarBorder extends PolarBehavior(BeruangBehavior(GestureEventListeners(PolymerElement))){static get template(){return html`
        ...
        `}static get properties(){return{north:{type:Number,value:0},northResizable:{type:Boolean,value:!1},south:{type:Number,value:0},southResizable:{type:Boolean,value:!1},west:{type:Number,value:0},westResizable:{type:Boolean,value:!1},east:{type:Number,value:0},eastResizable:{type:Boolean,value:!1}}}_stopDown(event){event.preventDefault();event.stopPropagation()}_resizer(path){for(let i=0;i<path.length;i++){if(path[i].classList.contains("resizer")){return path[i]}}return null}_resize(event){switch(event.detail.state){case"start":let path=event.path||event.composedPath&&event.composedPath(),resizer=this._resizer(path),id=resizer.id,arr=id.split("-"),region=arr[0];//north, south, east, west
