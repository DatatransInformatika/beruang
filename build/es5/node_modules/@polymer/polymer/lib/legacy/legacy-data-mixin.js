define(["exports","./class.js","../../polymer-legacy.js","../utils/mixin.js","../utils/templatize.js"],function(_exports,_class,_polymerLegacy,_mixin,_templatize){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.LegacyDataMixin=void 0;/**
@license
 ...
 */function wrapEffect(effect,fnName){if(effect&&effect.fn){var fn=effect.fn;effect.fn=function(){try{fn.apply(this,arguments)}catch(e){if(babelHelpers.instanceof(e,UndefinedArgumentError)){console.warn("Argument '".concat(e.arg,"'").concat(fnName?" for method '".concat(fnName,"'"):""," was undefined. Ensure it has a default value, or else ensure the method handles the argument being undefined."))}else{throw e}}}}return effect}/**
...
if(this._legacyUndefinedCheck&&1<vals.length){for(var i=0;i<vals.length;i++){if(vals[i]===void 0){// Break out of effect's control flow; will be caught in
// wrapped property effect function below
var name=args[i].name;throw new UndefinedArgumentError("Argument '".concat(name,"' is undefined."),name)}}}return vals}/**
...
_exports.LegacyDataMixin=LegacyDataMixin;_polymerLegacy.Polymer.Class=function(info,mixin){return(0,_class.Class)(info,function(superClass){return mixin?mixin(LegacyDataMixin(superClass)):LegacyDataMixin(superClass)})};// Apply LegacyDataMixin to Templatizer instances as well, and defer
 ...
 */var TemplatizeMixin=(0,_mixin.dedupingMixin)(function(superClass){/**
     * @constructor
     * @extends {HTMLElement}
     */var legacyBase=LegacyDataMixin(superClass),TemplateLegacy=/*#__PURE__*/function(_legacyBase){babelHelpers.inherits(TemplateLegacy,_legacyBase);function TemplateLegacy(){babelHelpers.classCallCheck(this,TemplateLegacy);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(TemplateLegacy).apply(this,arguments))}babelHelpers.createClass(TemplateLegacy,[{key:"_legacyUndefinedCheck",get:function get(){return this._methodHost&&this._methodHost._legacyUndefinedCheck}}]);return TemplateLegacy}(legacyBase);/**
     * @private
     */ /** @type {!Polymer_PropertyEffects} */TemplateLegacy.prototype._methodHost;return TemplateLegacy});_templatize.templatize.mixin=TemplatizeMixin;console.info("LegacyDataMixin will be applied to all legacy elements.\n"+"Set `_legacyUndefinedCheck: true` on element class to enable.")});