define(["exports","../polymer/lib/legacy/polymer.dom.js","../polymer/lib/utils/render-status.js"],function(_exports,_polymerDom,_renderStatus){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.IronDocViewerBehavior=void 0;/**
 * @polymerBehavior
 */var IronDocViewerBehavior={properties:{/**
     * The [Polymer
...
(0,_polymerDom.flush)();var anchor=this.fragmentPrefix+hash,element=this.$$("[anchor-id=\""+anchor+"\"]");if(element){// Don't scroll until we've drawn the next frame, otherwise our
// element might not know it's final screen position yet.
(0,_renderStatus.afterNextRender)(this,function(){element.scrollIntoView({behavior:"smooth"});// Highlight the section for a moment so we can tell where
 ...
 */_exports.IronDocViewerBehavior=IronDocViewerBehavior;function compareUnderscoresLast(a,b){var numA=numLeadingUnderscores(a),numB=numLeadingUnderscores(b);if(numA!==numB){return numA-numB}return a.localeCompare(b)}function numLeadingUnderscores(str){return str.match(/^_*/)[0].length}});