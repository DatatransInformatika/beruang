...
class Document {
    ...
    getFeatures(query = {}) {
//console.log('getFeatures', this._beruang_documentation);
    ...
    _listFeatures(result, visited, query) {
        ...
        for (const feature of this._localFeatures) {
            ...
            if (feature.kinds.has('import') && query.imported) {
                const imprt = feature;
			////ABDUL disabled:BEGIN
                //const isPackageInternal = imprt.document && !analysis_1.Analysis.isExternal(imprt.url);
			////ABDUL disabled:END
			////ABDUL inserted:BEGIN
                const isPackageInternal = imprt.document && !analysis_1.Analysis.isExternal(imprt.url, this._beruang_documentation);
			////ABDUL inserted:END
    ...
    _filterOutExternal(features) {
        ...
        for (const feature of features) {
			...
			////ABDUL disabled:END
			////ABDUL inserted:BEGIN
                analysis_1.Analysis.isExternal(feature.sourceRange.file, this._beruang_documentation)) {
			////ABDUL inserted:END
                continue;
            }
            result.add(feature);
