...
/////////ABDUL inserted:BEGIN
		let beruang = options.name.startsWith("beruang-");
		if(beruang) {
			options.name = options.name.replace(/^beruang-/, '');
		}
/////////ABDUL inserted:END
...
////////ABDUL disabled:END
////////ABDUL inserted:BEGIN
		let namearr = beruang ? options.name.split("-") : null;
        const buildDirectory = path.join(exports.mainBuildDirectoryName, namearr ? namearr[0] : buildName);
////////ABDUL inserted:END
