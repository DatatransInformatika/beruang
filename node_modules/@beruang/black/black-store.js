...
class BlackStore extends PolymerElement {
  ...
  getRecordByValue(primaryValue) {
      let index = this._recordIndex(primaryValue);
      return index>=0 ? this.recordset[index] : null;
  }

  getRecordByIndex(index) {
      let len = this.recordset.length;
      return (index=>0 && index<len) ? this.recordset[index] : null;
  }

  _addTuple(index, tuple, primaryValueArr, recordLen) {
      let primaryValue = this._checkTuple(tuple, primaryValueArr);
      let record = new Record(tuple);
  ...
  _recordIndex(primaryValue) {
      ...
      for(let i=0; i<this.recordset.length; i++) {
          ...
          let value = record.getValue(this.key);
  ...
  _primaryValueArray() {
      let primaryValueArr = [];
      for(let i=0; i<this.recordset.length; i++) {
          let value = this.recordset[i].getValue(this.key);
