...
@polymer
*/
class BrownTabManualDesktop extends BrownTabBehavior(BrownBehavior(BeruangBehavior(GestureEventListeners(PolymerElement)))) {
  static get template() {
    return html`
  ...
  ready() {
      ...
      let fMonitorCallback = (info) => {
          for(let i=0;i<info.addedNodes.length;i++){
              ...
              let nextActiveTab = activeTabId ? this._getTabItemByDataTab(activeTabId) :null;
              if(nextActiveTab) {
                  let oldActiveTab = this._getActiveTabItemBy();
