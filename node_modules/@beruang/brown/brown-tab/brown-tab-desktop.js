...
////////////////////////fungsi untuk menggeser tab lain (next / previous), jika suatu tab ditutup:START/////////////
  ...
  //forward
    let element = el; //element -> el, el jangan pernah digantikarena el adl patokan
    while(
      (sibling = element.nextElementSibling) //dptkan next elemen sibling //null atau tidak,kalau null keluar dari while
    ) {
      if( sibling.nodeName=="BROWN-TABITEM-DESKTOP" ) {//syarat 1: sibling harus <brown-tab-desktop></brown-tab-desktop>
        if( !callback || callback(sibling) ) {//syarat 2: jika callback return true, maka sibling memenuhi syarat
          return sibling;
        }
      }
      element = sibling; //menggeser pointer elemen dri el ke sibling, sibling ke sibling, sampai siblingnya null
    }

  //backward
    element = el; //geser pointer elemen kembali ke el, el jangan pernah digantikarena el adl patokan
    ...
    return null;//sibling <brown-tab-desktop></brown-tab-desktop> sudah tidak ada (tdkada yg memenuhi syarat)
  ...
  _getContentByDataTab(tabId) {
    return this._getLightDOM(true, (el)=>{
      let data = el.nodeName!="BROWN-TABITEM-DESKTOP" ? el.getAttribute("data-tab") : null;
      return tabId==data;
    });
  }

  _getLightDOM(forward, callback) {
    let el = forward ? this.firstElementChild : this.lastElementChild;
    while(el) {
      if(callback(el)) {
        return el;
      }
  ...
  _evaluateScrollDo(scrollerId) {
    ...
    if(scrollerId=="#post-scroll") {
      let tabItem = this._getLightDOM(false, (el)=>{//get last tabItem
      ...
      let tabItem = this._getLightDOM(true, (el)=>{//get first tabItem
        return el.nodeName=="BROWN-TABITEM-DESKTOP";
      });
      if(tabItem) {
        let left = tabItem.offsetLeft - container.scrollLeft;
  ...
  _preScroll(e) {
    let tabItem = this._getLightDOM(true, (el)=>{//search first tabItem
      return el.nodeName=="BROWN-TABITEM-DESKTOP";
    });
    if(tabItem) {
      let container = this.shadowRoot.querySelector("#header-item");
  ...
  _postScroll(e) {
    let tabItem = this._getLightDOM(false, (el)=>{//search last tabItem
      return el.nodeName=="BROWN-TABITEM-DESKTOP";
    });
    if(tabItem) {
      let container = this.shadowRoot.querySelector("#header-item");
