...
class BrownTabDesktop extends BrownBehavior(BeruangBehavior(GestureEventListeners(PolymerElement))) {
  ...
  _evaluateScroll() {
    setTimeout( ()=>{
      this._evaluatePostScroll();
      setTimeout( ()=>{
        this._evaluatePreScroll();
      }, 100);
    }, 100);
  }
  ...
  _evaluatePostScroll() {
    let container = this.shadowRoot.querySelector("#header-item");
    let overflowEl = this._getLightDOM((el)=>{
        if(el.nodeName=="BROWN-TABITEM-DESKTOP") {
          let right = el.offsetLeft + el.offsetWidth - container.scrollLeft;
          return right > container.offsetWidth;
        }
        return false;
    });
    let scroller = this.shadowRoot.querySelector("#post-scroll");
    if(overflowEl) {
  ...
  _evaluatePreScroll() {
    ...
    let overflowEl = this._getLightDOM((el)=>{
        if(el.nodeName=="BROWN-TABITEM-DESKTOP") {
          let left = el.offsetLeft - container.scrollLeft;
          return left < 0;
        }
        return false;
    });
    let scroller = this.shadowRoot.querySelector("#pre-scroll");
    if(overflowEl) {
      scroller.classList.add("scroll");
    } else {
      scroller.classList.remove("scroll");
  ...
  _preScroll(e) {
    let leftest = 999;
    let container = this.shadowRoot.querySelector("#header-item");
    this._getLightDOM((el)=>{
      if(el.nodeName=="BROWN-TABITEM-DESKTOP") {
        let left = el.offsetLeft - container.scrollLeft;
        leftest = Math.max(leftest, left);
      }
      return false;
    });
    let scrollSize = container.offsetWidth/5;
    let offset = 0 - leftest;
    if(offset>0) {
      scrollSize = Math.min(scrollSize, offset);
    }
  ...
  _postScroll(e) {
    ...
    this._getLightDOM((el)=>{
      if(el.nodeName=="BROWN-TABITEM-DESKTOP") {
        let right = el.offsetLeft + el.offsetWidth - container.scrollLeft;
        rightest = Math.max(rightest, right);
      }
      return false;
    });
    let scrollSize = container.offsetWidth/5;
    let offset = rightest - container.offsetWidth;
    if(offset>0) {
      scrollSize = Math.min(scrollSize, offset);
    }
    container.scrollLeft += scrollSize;
