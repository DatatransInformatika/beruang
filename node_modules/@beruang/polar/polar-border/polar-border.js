    ...
    _regionSize(connected, fixedSize, size, cssProp) {
        if(connected) {
            if(size>=1) {
                ...
                let region = arr[2];//north, south, west, east
                let calcSize = size * ( (region=="north" || region=="south") ? this.offsetHeight : this.offsetWidth);
                let obj = {};
        ...
        case 'track':
            if(this._region) {
                ...
                switch(this._region.id) {
                    case "north":
                    case "south":
                        offset = event.detail.dy * (this._region.id=="south" ? -1 : 1);
                        size = this._regionSize + offset;
                        ...
                        //consider northMinHeight or southMinHeight
                        minSize = this.get(this._region.id + "MinHeight");
                    ...
                    case "west":
                    case "east":
                        offset = event.detail.dx * (this._region.id=="east" ? -1 : 1);
                        size = this._regionSize + offset;
                        ...
                        //consider westMinWidth or eastMinWidth
                        minSize = this.get(this._region.id + "MinWidth");
                        size = Math.max(size, minSize);
                        //css property
                ...
                if( cssProp.length > 0 ) {
                    let obj = {};
                    obj[cssProp] = size + "px";
                    this.updateStyles(obj);//apply height or width
                }
            }
            break;
        case 'end':
