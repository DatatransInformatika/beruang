...
class PolarBorder extends PolarBehavior(BeruangBehavior(GestureEventListeners(PolymerElement))) {
    ...
    static get observers() {
        return [
            '_regionSize(connected, north, "--north-height")',
            '_regionSize(connected, south, "--south-height")',
            '_regionSize(connected, west, "--west-width")',
            '_regionSize(connected, east, "--east-width")'
        ];
    }
    ...
    _regionSize(connected, size, cssProp) {
        if(connected) {
            if(size>=1) {
                let obj = {};
                obj[cssProp] = size+"px";
                ...
                let fWidth = orientation=="horizontal" ? ()=>{ return this.offsetWidth; } : null;
                let fHeight = orientation=="vertical" ?  ()=>{ return this.offsetHeight; } : null;
                let fCallback = (w,h)=>{
                    let calcSize = size * (orientation=="horizontal" ? w : h);
                    let obj = {};
    ...
    _eastSize(connected, east) {
        if(connected) {
            if(east>=1) {
                this.updateStyles({"--east-width":east+"px"});
            } else if(east>0) {
                let fWidth = ()=>{ return this.offsetWidth; };
                let fCallback = (w,h)=>{
                    let size = east * w;
                    this.updateStyles({"--east-width":size+"px"});
                }
    ...
    _regionEnter(event) {
        ...
        switch(region) {
            case "north":
                this._setNorthCollapser(true);
                break;
            case "south":
                this._setSouthCollapser(true);
                break;
            case "west":
                this._setWestCollapser(true);
                break;
            case "east":
                this._setEastCollapser(true);
                break;
        }
    }
